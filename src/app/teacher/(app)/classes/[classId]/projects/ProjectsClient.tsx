    ...
    <div className="mt-6 flex flex-col gap-6">
      <div className="rounded-2xl border border-slate-200 bg-white p-5">
        <div className="flex items-center justify-between">
          <h2 className="font-semibold text-slate-900">قائمة المشاريع</h2>
          {!showForm && (
            <button
        ...
        {showForm && (
          <form onSubmit={handleAdd} className="mt-4 flex flex-col gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4">
            <div className="flex flex-col gap-1">
              ...
              <input name="title" required className="rounded-lg border border-slate-300 px-3 py-2 text-sm bg-white" />
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-xs text-slate-600">وصف (اختياري)</label>
              <textarea name="description" rows={2} className="rounded-lg border border-slate-300 px-3 py-2 text-sm bg-white" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="flex flex-col gap-1">
                <label className="text-xs text-slate-600">الدرجة القصوى</label>
                <input name="maxScore" type="number" min={1} defaultValue={100} required className="rounded-lg border border-slate-300 px-3 py-2 text-sm bg-white" />
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-xs text-slate-600">تاريخ التسليم (اختياري)</label>
                <input name="dueDate" type="date" className="rounded-lg border border-slate-300 px-3 py-2 text-sm bg-white" />
              </div>
            </div>
