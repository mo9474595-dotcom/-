  ...
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-4 px-4 py-10">
      <div className="rounded-2xl bg-white p-6 text-center shadow-sm">
        <h1 className="text-xl font-bold text-brand-navy-dark">{data.examTitle}</h1>
        <p className="mt-1 text-sm text-slate-500">مراجعة تفصيلية للنتيجة — {data.studentName}</p>
          ...
          return (
            <div key={q.id} className="rounded-2xl bg-white p-5 shadow-sm">
                  ...
                  {q.choices.map((c) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span>{c.text}</span>
                        {chosen && !c.isCorrect && (
                          <span className="mr-auto text-xs text-red-600">إجابتك</span>
                        )}
                      </div>
                    );
                  })}
                  {!q.selectedChoiceId && (
                    <p className="text-xs text-amber-600">لم تُجب على هذا السؤال</p>
                  )}
                </div>
              ) : (
                <div className="mt-3 flex flex-col gap-2 text-sm">
                  <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                    <p className="text-xs font-medium text-slate-500">إجابتك</p>
                  ...
                  {q.correctAnswer && (
                    <div className="rounded-lg border border-green-200 bg-green-50 px-3 py-2">
                      <p className="text-xs font-medium text-green-700">إجابة نموذجية</p>
                      <p className="mt-0.5 text-green-800">{q.correctAnswer}</p>
                    </div>
                  )}
                </div>
              )}
              ...
              <p className="mt-3 text-xs text-slate-500">
                {graded ? (
                  <>
                    الدرجة: <span className="font-medium text-slate-700">{q.pointsAwarded}</span> من{" "}
                    {q.points}
                  </>
                ) : (
